...
router.post("/", authenticateJWT, async (req, res) => {
  try {
    ...
    const { courseId, title, description, price, instructor, category, tags } =
      req.body; // <-- Added 'tags' here

    if (!courseId || !title || !price || !instructor || !category) {
      return res
        .status(400)
        .json({ message: "All required course fields are missing." });
    }
...
router.get("/", async (req, res) => {
  try {
    const { search, category, instructor, minPrice, Price, tags } = req.query; // Get query parameters
    ...
    if (minPrice || Price) {
      ...
      if (Price) {
        query.price.$lte = parseFloat(Price); // $lte: less than or equal to
      }
      // Basic validation for price range
      if (isNaN(query.price.$gte) || isNaN(query.price.$lte)) {
        return res
          .status(400)
          .json({ message: "Invalid price range provided." });
      }
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to retrieve courses with applied filters" });
  }
});
  ...
  try {
    if (req.user.role !== "instructor") {
      return res
        ...
        .json({
          message:
            "Access denied. Only instructors can view their own courses.",
        });
    }

    // --- NEW: Add search/filter to my-courses as well (optional, but useful) ---
    const { search, category, minPrice, Price, tags } = req.query;
  ...
  try {
    if (req.user.role !== "instructor") {
      return res
        .status(403)
        .json({
          message: "Access denied. Only instructors can update courses.",
        });
    }
    ...
    if (
      ...
      (!Array.isArray(updateData.tags) ||
        !updateData.tags.every((tag) => typeof tag === "string"))
    ) {
      return res
        .status(400)
        .json({ message: "Tags must be an array of strings." });
